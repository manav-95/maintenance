...
export default function AdminMaintenanceDashboard() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-8">
      {/* Header */}
      <div className="mb-10">
        <h1 className="text-4xl font-bold text-gray-900">Maintenance Dashboard</h1>
        ...
        <div className="bg-white rounded-lg shadow-md p-8 border-l-4 border-purple-500 hover:shadow-lg transition">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-2">₹{maintenanceStats.spent.toLocaleString()} / ₹{maintenanceStats.monthlyBudget.toLocaleString()}</p>
            </div>
      ...
      <div className="flex gap-6 mb-8 border-b border-gray-200">
        <button
          ...
          className={`px-4 py-3 font-semibold text-base transition ${activeTab === 'overview' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-600 hover:text-gray-900'}`}
        >
          Analytics & Overview
        </button>
        <button
          onClick={() => setActiveTab('recent')}
          className={`px-4 py-3 font-semibold text-base transition ${activeTab === 'recent' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-600 hover:text-gray-900'}`}
        >
          Recent Maintenance
        </button>
      </div>
              ...
              {recentMaintenance.map((item) => (
                <tr key={item.id} className="border-b hover:bg-gray-50 transition">
                  ...
                  <td className="px-8 py-5">
                    <span className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold ${getStatusColor(item.status)}`}>
